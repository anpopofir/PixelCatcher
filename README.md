...
1. Clonar el repositori i preparar l'entorn: git clone https://github.com/anpopofir/PixelCatcher.git cd PixelCatcher python -m pip install pillow pyautogui pyinstaller
2. Generar l'executable per a Windows (.exe): Per compilar el programa en un sol fitxer portable i sense la finestra de la consola, executa: python -m PyInstaller --noconsole --onefile --clean --icon=icona.ico pixelcatcher.py
🤝 Contribucions
